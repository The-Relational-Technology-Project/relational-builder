...
- **Community access** -- invited pilot builders get free Claude building via RTP's subsidized key, gated server-side with generous weekly budgets — no API key, no credit card. Claude Opus 5.5 (the strongest builder on our bench) is the default for builds and edits, Claude Fable 5.1 for planning; the weekly budget also covers Opus 5, Opus 4.8, Sonnet 5, and Haiku 4.5, and picking a model in the picker pins it for that project. If Anthropic has an outage, community builds automatically fail over to a covered non-Anthropic model so a workshop never stalls
- **Community Cloud** -- one click gives any project a free shared data store hosted by RTP (10 backends per builder, 200MB shared across them) with built-in neighbor email-code sign-in — no database accounts, no SQL
...
- **AI artwork** -- when a build needs imagery no photo exists for yet (flyer art, custom icons), generate it in place through the proxy's image lane; real local photos still come first, and community members are covered by the same weekly budget as chat
...
- **Community Hosting** -- one-click free hosting from RTP (10 sites per builder, no platform accounts or tokens) with privacy-friendly visit counts; Netlify/Vercel still available for bigger needs
...
- **Model-agnostic** -- latest models from Anthropic (Claude Opus 5.5 by default, plus Fable 5.1, Opus 5, Opus 4.8, Sonnet 5, Haiku 4.5), Google (Gemini 3.5), OpenAI (GPT-5.5), Together AI, OpenRouter, or any OpenAI-compatible endpoint
...
- **Claude** -- get a key at [console.anthropic.com](https://console.anthropic.com) (Opus 5.5, Fable 5.1, Opus 5, Opus 4.8, Sonnet 5, Haiku 4.5)
