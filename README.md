...
- **Community access** -- invited pilot builders get free Claude building via RTP's subsidized key, gated server-side with generous weekly budgets — no API key, no credit card. Claude Opus 5 (the strongest builder on our bench) is the default for builds and edits, Claude Fable 5.1 for planning; the weekly budget also covers Opus 4.8, Sonnet 5, and Haiku 4.5, and picking a model in the picker pins it for that project. If Anthropic has an outage, community builds automatically fail over to a covered non-Anthropic model so a workshop never stalls
