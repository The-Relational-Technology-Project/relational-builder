...
const PORTAL_NOTES: Record<string, string[]> = {
  ...
  ckan: [
    ...
    '- **Read the schema, then read the values — never invent a field.** `get_schema` returns the exact field list for a resource in one call; call it before writing any query, and plan only around fields it actually names. (San José\'s 311 export, for instance, has no ZIP and no street address at all — only `Latitude`/`Longitude` — so a plan built around "filter by zip code" cannot work no matter how reasonable it sounds.) Then check the values too: `SELECT "Field", count(*) … GROUP BY "Field"` on any column you intend to filter. Civic exports scatter the meaningful words — San José\'s `Category` holds `Painted Wall`, `Light Pole`, `None`, while the terms a person would filter on (`Graffiti`, `Illegal Dumping`, `Pothole`) live in `Service Type`. An allowlist aimed at the wrong column matches nothing and fails silently.',
    '- **Every column is text — including numbers, coordinates, and dates.** Cast before comparing, always. A latitude/longitude bounding box written without `::float` returns *zero rows* on a real San José query, because negative longitudes compare lexically (`\'-121.92…\' < \'-121.93\'`); the filter excludes everything and the app renders a confident, wrong empty state. Same for dates (`to_timestamp("Date Created", \'MM/DD/YYYY HH12:MI:SS AM\')`) and any numeric threshold — check the format in a sample row first, because sorting the raw string sorts alphabetically and lies.',
    ...
    '- **The portal allowlists SQL functions.** Anything outside the plain set comes back as an HTTP 403 naming the function (`Not authorized to call function FILTER`), not as a syntax error. Stay with `SELECT`/`WHERE`/`GROUP BY`/`ORDER BY` and `count`/`min`/`max`/`sum`/`CASE`, and read a 403 as "rewrite the query", not "the endpoint is down".',
...
export function formatCivicDataForPrompt(matches: CityDataEndpoint[]): string {
  ...
  return [
    ...
    '- **Prove the filter before you trust an empty result.** Run the query once with the filters stripped off. If the unfiltered version returns rows and the filtered one returns none, the filter is what is broken — a field that doesn\'t exist, an uncast comparison, a value that lives in a different column — not the neighborhood being quiet. A civic list that has never once returned a row is a bug, and shipping it as an empty state tells someone their street is fine when nobody has actually looked.',
    ...
    '- **Empty is not the same as all-clear.** "The query ran and found nothing", "the query failed", and "this has never returned an answer yet" are three different states and must read as three different screens. Never write reassuring copy for a zero-row civic result — no "all clear", no "nothing waiting right now", no "check back after the next city sweep". The likeliest cause of an empty civic list is a broken filter, and a warm empty state turns that bug into a confident false claim about someone\'s neighborhood. If the tool cannot tell the difference, say so plainly instead.',
