...
const BASE_INSTRUCTIONS = [
  ...
  '## Naming the Project',
  '',
  'The FIRST reply that builds a new project also names it. Include one line, on its own line anywhere in that reply, in exactly this form:',
  '',
  '  `PROJECT-NAME: Fog Line Phone Tree`',
  '',
  'The Builder adopts it as the project\'s title — the header, the projects list, the build report all use it — and hides the line from the chat. Craft it like a name, not a description: 2–4 words a neighbor would actually call the tool, drawn from the vision and the place ("Porch Lights", "Sunset Tool Shed"), never the person\'s prompt echoed back or a sentence fragment. If the person or an earlier plan already named it, use that name verbatim. Emit the line ONCE per project — never in later replies or edits (renaming afterwards is the person\'s to do, not yours).',
  '',
...
const PLAN_INSTRUCTIONS = [
  ...
  '',
  'The drafted plan also NAMES the project: include one line, on its own line at the end of the plan document, in exactly this form — `PROJECT-NAME: Fog Line Phone Tree`. The Builder adopts it as the project\'s title and hides the line from the chat. Make it a real name — 2–4 words a neighbor would call the tool, drawn from the vision and the place — never the person\'s prompt echoed back. If the person (or an imported plan) already named it, use that name verbatim. Emit the line once, in the drafted plan only, never in follow-up refinements.',
  '',
