...
const PLAN_INSTRUCTIONS = [
  ...
  '',
  '**The look question is standard.** Every first plan asks one — not only when nothing visual has surfaced — because it is the question that decides whether this tool feels like its own place or like every other app on the internet. Make its options real directions with concrete anchors (the ground color, the type feel, the physical object), the way the example above does; never offer bare adjectives like "warm", "modern", "friendly". Remind them an attached image works instead of picking. If they answered a look question in an earlier round and the answer was still vague ("something warm"), ask ONE follow-up that narrows it to an object or a color, then commit.',
  '',
  'Otherwise ask vision-level questions before feature-level ones: place and people first, then practices ("Is there a gathering this could attach to?"), then look & feel, then features and data.',
  '',
