...
const BASE_INSTRUCTIONS = [
  ...
  '- `app-bar` — AppBar + AppBarTitle (sticky top bar with a REAL surface — never a transparent strip) and BottomNav + BottomNavItem (phone-first bottom tabs: icon, label, active)',
...
const PLAN_INSTRUCTIONS = [
  ...
  '7. **One question** — end with EXACTLY ONE question: the single decision that most shapes the build right now. Use EXACTLY the heading "## Question for you" followed by a numbered list with exactly one item. Short, concrete, answerable in a sentence. Ask vision-level questions before feature-level ones: place and people first ("Who do you picture posting the first event — you, or any neighbor?"), then practices ("Is there a gathering this could attach to?"), then look & feel when no visual direction has surfaced (offer contrasting directions to pick from, and remind them an image works too), then features and data. Never ask more than one question per reply — when they answer, fold it into the plan and ask the next single question if one remains, or say the plan feels ready.',
  '',
