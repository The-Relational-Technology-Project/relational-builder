...
export interface ProjectLineage {
  ...
  source: 'rtp-studio-plan' | 'remix' | 'prompt' | 'repo-import' | 'dream' | 'builder-profile' | null;
...
interface ProjectState {
  ...
  /** Least-recently-touched first — the prompt snapshot caches on byte prefix */
...
export const useProjectStore = create<ProjectState>()(persist((set, get) => ({
  ...
  getFilesForPrompt: () => get().fs.getFilesForPrompt(),
