...
export const COMMUNITY_CLOUD_GUIDANCE = [
  ...
  '  ```',
  '  Documents are JSON objects up to 32KB; lists return up to 100; each app backend holds up to 5000 documents, with 200MB of storage shared across all of a builder\'s backends on the free community tier. Prefer this over asking the user to set up Supabase for simple shared data (boards, calendars, signups, RSVPs). The builder can browse and moderate everything stored here in the Cloud tab.',
  '',
  ...
  '',
  '- **Typed collections (optional, for data with real shape).** When a collection has a clear structure — applications, RSVPs, listings with amounts and statuses — declare it in a project file `/cloud-schema.json` and the platform validates every write server-side and enables filtered queries. Collections you don\'t declare stay schemaless and work as before.',
  '  ```json',
  '  {"collections": {',
  '    "offers": {"fields": {',
  ...
  '  }}',
  '  ```',
  '  Field types: string | number | boolean | array | object; options: required, unique, maxLength. The Builder syncs this file to the cloud automatically after each build — write it once and edit it as the app grows. Rejected writes come back as `{error}` with a friendly message (422 invalid, 409 duplicate) — surface them next to the form field, not as a crash.',
  '  Declared collections can be queried with filters instead of fetching everything:',
  '  ```javascript',
  '  // where ops: eq, neq, gt, gte, lt, lte, contains (case-insensitive substring)',
  '  await cloudRequest("query", "offers", {',
  '    where: [{ field: "amount", op: "lte", value: 50 }, { field: "title", op: "contains", value: "tool" }],',
  '    order: { field: "amount", dir: "asc", numeric: true },   // numeric: true for number fields',
  '    limit: 25,',
