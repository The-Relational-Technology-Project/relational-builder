...
//
// ⚠️ This function deploys to the RTP WEBSITE Supabase project
// (odowkowcinyoxejyzhwl — the commons backend shared with Relational Tech
// Studio), NOT to the Relational Builder project like the rest of
// supabase/functions/. It's mirrored here because the Builder's contribute
// flows depend on it; deploy with the Management API against that project.
//
...
// Contribution mail goes to the reviewing steward directly, with the other
// stewards copied — a shared alias hid whether anyone actually saw it.
const STEWARD_EMAIL = Deno.env.get("STEWARD_EMAIL") ?? "deborah@relationaltechproject.org";
const STEWARD_CC = (Deno.env.get("STEWARD_CC_EMAILS") ?? "josh@relationaltechproject.org")
...
// "reference" — a link or doc worth adding to the reading room (the
// Builder's Resource category submits these).
const ALLOWED_TYPES = [
  "tool",
  "story",
  "recipe",
...
serve(async (req)=>{
  ...
  if (!summary && !content) {
    return new Response(JSON.stringify({
      error: "Provide a summary or body"
    }), {
      status: 400,
      headers: jsonHeaders
    });
  ...
  if (!builderName) {
    return new Response(JSON.stringify({
      error: "builder_name is required — the commons credits contributors"
    }), {
      status: 400,
      headers: jsonHeaders
    });
  ...
  const { count: globalCount } = await supabase.from("commons_contributions").select("id", {
    count: "exact",
    head: true
  }).gte("submitted_at", hourAgo);
  if ((globalCount ?? 0) >= GLOBAL_HOURLY_LIMIT) {
    return new Response(JSON.stringify({
      error: "The contribution queue is busy right now — please try again in an hour."
    }), {
      status: 429,
      headers: jsonHeaders
    });
  }
  const { count: ipCount } = await supabase.from("commons_contributions").select("id", {
    count: "exact",
    head: true
  }).gte("submitted_at", hourAgo).eq("proposed_metadata->>submit_ip", ip);
  if ((ipCount ?? 0) >= PER_IP_HOURLY_LIMIT) {
    return new Response(JSON.stringify({
      error: "You've submitted a few contributions recently — please give the stewards a moment to catch up and try again in an hour."
    }), {
      status: 429,
      headers: jsonHeaders
    });
  ...
  const { data: studioRow } = await supabase.from("commons_studios").select("slug").eq("slug", sourceSlug).maybeSingle();
  if (!studioRow) {
    const { error: studioErr } = await supabase.from("commons_studios").insert(studioSlug ? {
      slug: sourceSlug,
      ...
      notes: "Auto-registered from a studio-framed contribution (Relational Builder). Every item waits in commons_contributions for steward review."
    } : {
      ...
      publish_key_hash: "managed-by-submit-contribution",
      steward_email: STEWARD_EMAIL,
      notes: "Contributions submitted by builders through MCP-connected AI tools. Every item waits in commons_contributions for steward review."
    });
    if (studioErr) {
      return new Response(JSON.stringify({
        error: "Failed to register contribution channel",
        details: studioErr.message
      }), {
        status: 500,
        headers: jsonHeaders
      });
  ...
  const { data: inserted, error: insertErr } = await supabase.from("commons_contributions").insert({
    ...
    proposed_metadata: {
      ...
      ...relatedItemSlug ? {
        related_item_slug: relatedItemSlug
      } : {},
      ...studioSlug ? {
  ...
  await supabase.from("commons_feed_events").delete().eq("event_type", "contribution_received").eq("payload->>contribution_id", inserted.id);
